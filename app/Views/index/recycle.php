...
<?php if (!empty($recycle_files)) : ?>
    <div class="ms-2 me-auto mt-2 d-flex flex-wrap" style="border-radius: 9px">
        <?php for ($i = 0; $i < sizeof($recycle_files); $i++) : ?>
            <a class="files list-group-item list-group-item-action list-group-item-light me-3 mb-2 mt-1 d-flex flex-column" fid="<?=$recycle_files[$i]["id"]?>" state="<?=$recycle_files[$i]["state"]?>" recycle>
                <span class="fw-bold fs-6 text-truncate"><?= esc($recycle_files[$i]["filename"]); ?></span>
                <time class="font-monospace" style="font-size: xx-small"><?= esc($recycle_files[$i]["filedate"]); ?></time>
            </a>
        <?php endfor; ?>
    </div>
<?php else : ?>
